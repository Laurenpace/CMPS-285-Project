...
namespace StarterProject.Api.Controllers
{
    ...
    public class UsersController : ControllerBase
    {
        ...
        [AllowAnonymous]
        [HttpGet("[controller]/authenticate-token")]
        [ProducesResponseType(typeof(UserAuthenticationGetDto), (int)HttpStatusCode.OK)]
        public IActionResult AuthenticateToken([FromQuery] string token)
        {
            var userAuth = _userService.AuthenticateToken(token);
        ...
        [AllowAnonymous]
        [HttpPut("[controller]/{userId:int}/logout")]
        ...
        public IActionResult Put(int userId)
        {
          //  _userService.Logout(userId)
                /*
                 Logout to IuserService interface
                 Add Logout Function to UserService
                 Logout Function should delete the old token with that userId out of the tokens table
